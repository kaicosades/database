...
//Подключили при создании проекта
using System.Data;
...
namespace Library
{
    class Library
    {
        ...
        public Library(string connectionString)// 2 38
        {
            this.connectionString = connectionString;
            connection = new SqlConnection(connectionString); // Удаление пробелов в App после Encrypt решает проблему ключевого слова
        ...
        public void InsertAuthor(string first_name, string last_name)
        {
            try
            {
                ...
                END"; //Значок доллара, чтобы интерполяция работала(чем от собаки отличается)
                   //@- raw строка "Чтобы можно было разносить как угодно"
        ...
        { 
            try
            {
                ...
                while (rdr.Read())
                {
                    Console.WriteLine($"{rdr[0].ToString().PadRight(10)} {rdr[1].ToString().PadRight(15)} {rdr[2].ToString().PadRight(15)}");//PadRight - вывод строки с указанным колличеством символов
                }
            }
            finally
            {
