DECLARE @start_lesson_id BIGINT= -- ид первой пары выполняющей след условия
(
...
DECLARE @end_lesson_id BIGINT= -- ид первой пары выполняющей след условия
(
...
BEGIN
--	DECLARE @studenst TABLE (student_id INT)
...
--------------------комментируется потому что при повторном запросе создается заного
-- f5 обрабатывает выделенный код

	--SELECT * FROM #CurrentGroup
	--DECLARE @id INT = (SELECT TOP 4 MAX(stud_id) FROM Students JOIN Groups ON [group] = group_id WHERE group_name = 'PD_212') 
	
	--SELECT TOP 1 @id =  #CurrentGroup.stud_id FROM #CurrentGroup
	--SELECT TOP 1  @id = (SELECT stud_id FROM Students JOIN Groups ON [group] = group_id WHERE group_name = 'PD_212')
